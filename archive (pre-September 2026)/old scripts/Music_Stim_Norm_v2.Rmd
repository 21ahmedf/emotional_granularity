...
stimulus_tracking <- read.csv("../stimuli/sachs/SourceData_1.csv")
...
```{r}
# Read stimulus tracking data
stimulus_tracking <- read.csv("../stimuli/sachs/cropped_stims/stim tracking sheets/Stimulus Tracking.csv")
...
# Apply function to different data subsets
process_data(neg_neg_portions, 3:4, "Negative Valence Negative Arousal")
process_data(neg_pos_portions, 17:30, "Negative Valence Positive Arousal")
process_data(pos_neg_portions, 54:69, "Positive Valence Negative Arousal")
process_data(pos_pos_portions, 34:50, "Positive Valence Positive Arousal")
```
...
# Read stimulus tracking data
stimulus_tracking <- read.csv("../stimuli/sachs/cropped_stims/stim tracking sheets/Stimulus Tracking.csv") #%>%
  #subset(file.name=="3_pieceB_joyous_anxious_sad_1" | file.name=="11_pieceA_calm_anxious_sad_4")
...
process_data <- function(portions, tracking_indices, title) {
  ...
  # Create plot for Valence vs. Arousal
  combined_plot <- ggplot(extracted_sections, aes(x = Valence, y = Arousal, group = SectionID)) +
    geom_point(aes(shape = Type)) +  # Distinguish start and end points with shapes
    geom_line() +  # Connect start and end points
    labs(
         x = "Normed Valence Rating",
         y = "Normed Arousal Rating") +
    theme_classic() +
    geom_vline(xintercept=0,color="red", linetype="dashed")+
    geom_hline(yintercept=0,color="red", linetype="dashed")+
    scale_shape_manual(values = c("Start" = 15, "End" = 17)) + # Using different shapes for start and end
    theme(
...
master <- rbind(neg_pos_portions,neg_neg_portions, pos_neg_portions,pos_pos_portions)

process_data(master, c(3:5,9,2,21,18,19,25,27,66,67,69,55,56,62,36,41,47,48), "Selected")
