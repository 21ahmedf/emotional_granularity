...
### EXCLUDE IN-PERSON PARTICIPANTS THAT HAVE SCANS
```{r}
...
plot_emotion_means <- function(data, title, file_name, type) {
  ...
  plot <- ggplot(summary, aes(x = variable, y = mean_value)) +
    ...
    strip.text = element_text(family="Arial",size=24,color='black') )+
    theme_bw()
  
  ggsave(file_name, plot, width = 9, dpi=1000)
...
music_negative_summary <- plot_emotion_means(music_negative, "Music Negative Emotion Means", "../results/rating-task/Music_Negative_Means_3_10_25.png", "Music")
music_positive_summary <- plot_emotion_means(music_positive, "Music Positive Emotion Means", "../results/rating-task/Music_Positive_Means_2_12_25.png", "Music")
pic_negative_summary <- plot_emotion_means(pic_negative, "Picture Negative Emotion Means", "../results/rating-task/Picture_Negative_Means_2_12_25.png", "Picture")
pic_positive_summary <- plot_emotion_means(pic_positive, "Picture Positive Emotion Means", "../results/rating-task/Picture_Positive_Means_2_12_25.png", "Picture")
...
ggsave('../results/rating-task/Negative_Emotion_Means_2_12_25.png', neg_plot, width = 9)
ggsave('../results/rating-task/Positive_Emotion_Means_2_12_25.png', pos_plot, width = 9)
```
...
png("../results/rating-task/Emotion Response Percentages_2_12_25.png", width = 400, height = 400)
...
ggsave('../results/rating-task/Picture Emotion Correlations 2_12_25.png', pic_corr_plot, width = 9)
ggsave('../results/rating-task/Music Emotion Correlations 2_12_25.png', music_corr_plot, width = 9)
...
# Save the plot
ggsave('../results/rating-task/All_Emotion_Means_2_12_25.png', full_plot, width = 9)
```
...
# Save the plots
ggsave('../results/rating-task/Emotion_Category_Means_2_12_25.png', summary_plot, width = 9)


#plotting stimulus type (pic or music) + emotion average for valence
all_averages$Type1 <- ifelse(grepl("Music", all_averages$Type),"Music", "Image" )

summary_plot <- 
  ggplot(all_averages, aes(x = Category, y = Average.Response, color=Type1)) +
...
ggsave('../results/rating-task/Emotion_Category_Means_pervalencestim.png', summary_plot, width = 9, dpi=1000)
```
...
# Save the plot
ggsave('../results/rating-task/All_Stims_Means_3_10_25.png', all_stims_plotted, width = 14)
...
collapsed_plot <- ggplot(collapsed_stims, aes(x = Type, y = Mean_Value)) +
  geom_point() +
...
ggsave('../results/rating-task/Average_Stims_Means_2_12_25.png', collapsed_plot, width = 10)
```
...
ggcorrplot(pic_pos_corr, 
           ...
           show.diag = TRUE,
           colors = c("blue", "white", "red"), 
           #title = "Correlations between Positive Emotion Ratings to Pictures",
           tl.cex = 20,
           ggtheme = theme_classic())

ggsave('../results/rating-task/Picture Negative Emotion Correlations3_11_25.png', pic_neg_corr_plot, width = 9, dpi=1000)
ggsave('../results/rating-task/Music Negative Emotion Correlations3_11_25.png', music_neg_corr_plot, width = 9, dpi=1000)
ggsave('../results/rating-task/Picture Positive Emotion Correlations3_11_25.png', pic_pos_corr_plot, width = 9, dpi=1000)
ggsave('../results/rating-task/Music Positive Emotion Correlations3_11_25.png', music_pos_corr_plot, width = 9, dpi=1000)
...
create_histogram <- function(data, variables, title, output_file, labels = NULL) {

  # Convert data to long format for ggplot
  data_long <- data[, variables]
  data_long <- reshape2::melt(data_long, variable.name = "Variable", value.name = "Score")
  
  # If custom labels are provided, rename the variables
  if (!is.null(labels)) {
    data_long$Variable <- factor(data_long$Variable, 
                                 levels = variables,  # Original variable names
                                 labels = labels)     # New labels
  }
  
  # Create the histogram
  p <- ggplot(data_long, aes(x = Score, fill = Variable)) +
    geom_histogram(alpha = 0.5, position = "identity", bins = 30) +
    theme_classic() +
    labs(title = title, x = "Granularity Score", y = "Count", fill = "Stimulus Type") +
    theme(
      legend.position = "top",
...
create_scatter_plot(results_recode, "negpicICC", "negmusicICC", 
                    ...
                    "Negative Emotional\nGranularity (Music)", 
                    "", 
                    "../results/rating-task/final/Neg_Graph_Final_3_12.png")

create_scatter_plot(results_recode, "pospicICC", "posmusicICC", 
                    "Positive Emotional\nGranularity (Pictures)", 
                    "Positive Emotional\nGranularity (Music)", 
                    "", 
                    "../results/rating-task/final/Pos_Graph_Final_3_12.png")
...
#                     "../results/rating-task/Neg_Graph_2_12_25.png")
# 
...
#                     "../results/rating-task/Pos_Graph_2_12_25.png")
# 
...
#                     "../results/rating-task/Combined_Graph_2_12_25.png")
```
...
create_mean_comparison_plot <- function(data, title, output_file) {
  mean_comparison <- data %>%
    ...
    theme(axis.title.x = element_text(size = 20, face = 'bold'),
          ...
          #plot.title = element_text(size = 16, face = 'bold', hjust = 0.5)) +
    facet_grid(~name)
    #scale_x_discrete(labels = c('Negative Music', 'Negative Picture', 'Positive Music', 'Positive Picture', 'Combined Music', 'Combined Picture'))
  
  ggsave(output_file, mean_comparison, width = 10)
}
...
#                             '../results/rating-task/Mean_Comparisons_2_12_25.png')
...
create_mean_comparison_plot(results_recode, 
                            '', 
                            '../results/rating-task/final/Mean_Comparisons_Final_3_10.png')


results_recode1 <- results_recode %>%
    pivot_longer(negmusicICC:pospicICC)

results_recode1$stim <- str_replace(results_recode1$name, "ICC", "")
results_recode1$valence <- str_replace(results_recode1$stim, "music", "")
results_recode1$valence <- str_replace(results_recode1$valence, "pic", "")
results_recode1$stim <- str_replace(results_recode1$stim, "neg", "")
results_recode1$stim <- str_replace(results_recode1$stim, "pos", "")
results_recode1 <- 
  results_recode1 %>%
...
poster_means<-
  ggplot(data=results_recode1, aes(x=valence, y=value, color=stim))+
...
ggsave('../results/rating-task/final/Mean_Comparisons_Final_3_12.png', poster_means, width=10, dpi=1000)
```
...
summary_results <- summary_stats(results_recode)
