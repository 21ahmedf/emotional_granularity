...
granularity_scores_2 <- read.csv(file='../data/study2_granularityscores.csv')

pos_ratings <- read.csv(file='../data/study1final_positiveratings.csv')

neg_ratings <- read.csv(file='../data/study1final_negativeratings.csv')
```

#manipulation check
```{r}
#test interaction between stimulus type (music/picture), emotion rating valence (pos/negative), and stimulus valence
neg_ratingsperemotion <- neg_ratings %>%
  pivot_longer(gloomy:upset) 

pos_ratingsperemotion <- pos_ratings %>%
  pivot_longer(happy:relaxed)

pos_ratingsperemotion$rating_valence <- "positive"
neg_ratingsperemotion$rating_valence <- "negative"

master_ratingsperemotion <- rbind(pos_ratingsperemotion, neg_ratingsperemotion)

#assigning stimulus valence
...
pos_pic_data <- filter_by_type(master_ratingsperemotion, pos_pic, "stim")
pos_pic_data$stimulus_valence <- "positive"
neg_pic_data <- filter_by_type(master_ratingsperemotion, neg_pic, "stim")
neg_pic_data$stimulus_valence <- "negative"

pos_music_data <- filter_by_type(master_ratingsperemotion, pos_music, "stim")
pos_music_data$stimulus_valence <- "positive"
neg_music_data <- filter_by_type(master_ratingsperemotion, neg_music, "stim")
neg_music_data$stimulus_valence <- "negative"

master_ratingsperemotion <- rbind(pos_pic_data,neg_pic_data,pos_music_data,neg_music_data)

m1 <- lmer(data=master_ratingsperemotion, (value) ~ condition*rating_valence*stimulus_valence + (condition*stimulus_valence*rating_valence|Participant.Public.ID) + (1|stim),control=lmerControl(optimizer="bobyqa"))
anova(m1)

#simple test of means (i.e., seeing pos rated pos, neg rated neg)
test(emmeans(m1, ~ condition*rating_valence*stimulus_valence,lmerTest.limit = 23040, lmer.df = "satterthwaite"))

emmeans::test(emmeans(m1, pairwise ~ condition*rating_valence*stimulus_valence,lmerTest.limit = 23040,lmer.df = "satterthwaite"))
...
averages_pos <- pos_ratings %>%
  ...
  summarise(
     `Average\nRating` = mean(Response, na.rm = TRUE)
    )
...
neg_averages %>%
  mutate(condition = recode(condition, music = "Negatively-Valencely Music", pic = "Negatively-Valenced Images")) %>%
  ggplot(aes(x = Emotion, y = stim, fill = `Average\nRating`)) +
    geom_tile() +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black"), axis.text.y = element_text(color = "black"), plot.background = element_rect(fill = "white", color = NA),  # Set plot background to white
        panel.background = element_rect(fill = "white", color = NA),legend.title = element_text(size = 11), 
...
pos_averages <- pos_averages %>%
  mutate(condition = recode(condition, music = "Positively-Valenced Music", pic = "Positively-Valenced Images"))

neg_averages <- neg_averages %>%
  mutate(condition = recode(condition, music = "Negatively-Valenced Music", pic = "Negatively-Valenced Images")) 

averages_total <- rbind(pos_averages, neg_averages)

averages_total$stim <- gsub(".jpg", "", averages_total$stim)
averages_total$stim <- gsub(".mp3", "", averages_total$stim)

averages_total_plot <-
ggplot(averages_total, aes(x = Emotion, y = stim, fill = `Average\nRating`)) +
    geom_tile() +
    #ggtitle(paste("Average Responses by Emotion and Stimulus for", group_name)) +
    xlab("Emotion") +
    ylab("Stimulus") +
    scale_fill_gradient(low = "blue", high = "red") +  # Adjust color scheme as needed
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 30), 
  ...
  facet_wrap(~condition, scales = "free") +
  geom_vline(xintercept=5.5, color="black", linetype="solid", size=3)

ggsave('../heatmap_averagestudy1.png',averages_total_plot, dpi=1000, width=18, height=10.5)
```
...
ggplot(data=granularity_scores, aes(x=posmusicICC, y=pospicICC)) +
  ...
  xlab("Positive Emotional\nGranulartiy Score - Music") +
  ylab("Positive Emotional\n Granularity Score - Images") +
  theme(
    axis.title.x = element_text(family="Arial", size=30),
    axis.title.y = element_text(family="Arial",size=30),
    axis.text.x = element_text(family="Arial", size=26),
    axis.text.y = element_text(family="Arial",size=26))

ggsave('../figures/study1_posgrancor.png',dpi=1000, pos_granularity_plot, height=8)
...
ggplot(data=granularity_scores, aes(x=negmusicICC, y=negpicICC)) +
  ...
  geom_point(alpha=0.7) +
  geom_smooth(method="lm") +
  xlab("Negative Emotional\nGranulartiy Score - Music") +
  ylab("Negative Emotional\n Granularity Score - Images") +
  theme(
    axis.title.x = element_text(family="Arial", size=30),
    axis.title.y = element_text(family="Arial",size=30),
    axis.text.x = element_text(family="Arial", size=26),
...
ggsave('../figures/study1_neggrancor.png',dpi=1000, neg_granularity_plot, height=8)
...
granularity_scores_long %>%
  ...
  stat_summary(fun.data = 'mean_cl_normal', geom = 'errorbar', size=1.25, width = 0.1, alpha=0.7,position = position_nudge(x = .3, y = 0), color="red") +
  stat_summary(fun.data = 'mean_cl_normal', geom = 'pointrange',size=1.25, alpha=0.7,position = position_nudge(x = .3, y = 0), color="red")+
  theme_classic()+
  theme(legend.position = "none") +
  facet_wrap(~valence,scales = "free_x") +
  geom_jitter(alpha=0.7, width=0.075,size=3) +
  ...
  theme(
    ...
    axis.text.x = element_text(family="Arial", size=44),
    axis.text.y = element_text(family="Arial",size=44),
    strip.text = element_text(family="Arial",size=46))



ggsave('../figures/study1_magdifference.png',dpi=1000, mag_difference, width=14, height=12)
...
average_corr <-
master_posneg %>%
  group_by(condition, valence, Participant.Public.ID,arousal_dif) %>%
    summarise(
...
master_posneg %>%
  ...
  ggplot(aes(x=arousal_dif, y=cor, color=condition)) +
   stat_summary(fun.data = 'mean_se', geom = 'errorbar',alpha=0.7, width = 0.075, size=1.4,position = position_nudge(x = .3, y = 0)) +
  stat_summary(fun.data = 'mean_se', geom = 'pointrange', alpha=0.7,size=0.9,position = position_nudge(x = .3, y = 0))+
  theme_classic() +
  theme(legend.position = "none") +
  facet_wrap(~valence)+
  xlab("Arousal Level") +
  ...
  theme(
    ...
    axis.text.x = element_text(family="Arial", size=44),
    axis.text.y = element_text(family="Arial",size=44),
    strip.text = element_text(family="Arial",size=44))+
  scale_color_manual(values=c("#0072B2", "#D55E00" )) +
  scale_fill_manual(values=c("#0072B2","#D55E00")) +
  geom_jitter(alpha=0.5, width=0.1, data=average_corr,size=3.5)



ggsave('../figures/study1_arousal_differentiation.png',dpi=1000, arousal_differentiation, width=14, height=12)
```

##check to make sure removing online participants don't change pattern of results
```{r}
#granularity across pics + music
##positive
granuliarty_scores_online <- granularity_scores  %>% 
  filter(!startsWith((ID), 'TEAM'))

cor.test(granuliarty_scores_online$posmusicICC, granuliarty_scores_online$pospicICC)

##negative
cor.test(granuliarty_scores_online$negmusicICC, granuliarty_scores_online$negpicICC)

##combined
cor.test(granuliarty_scores_online$allmusicICC, granuliarty_scores_online$allpicICC)

#average differences
neg_averages_online <- neg_averages  %>% 
  filter(!startsWith((Participant.Public.ID), 'TEAM'))

m1 <- lmer(data=neg_averages_online, mean ~ condition + (1|Participant.Public.ID))
summary(m1)

pos_averages_online <- pos_averages  %>% 
  filter(!startsWith((Participant.Public.ID), 'TEAM'))

m1 <- lmer(data=pos_averages_online, mean ~ condition + (1|Participant.Public.ID))
summary(m1)

#arousal diff.
master_online <- master  %>% 
  filter(!startsWith((Participant.Public.ID), 'TEAM'))
test <- lmer(data=master_online, scale(cor) ~ condition*arousal_dif + (condition*arousal_dif|Participant.Public.ID))
summary(test)

master_neg_online <- master %>%
  filter(!startsWith((Participant.Public.ID), 'TEAM'))

test <- lmer(data=master_neg_online, scale(cor) ~ condition*arousal_dif + (condition*arousal_dif|Participant.Public.ID))
summary(test)


#checking patterns of in-person data
#granularity across pics + music
##positive
granuliarty_scores_inperson <- granularity_scores  %>% 
  filter(startsWith((ID), 'TEAM'))

cor.test(granuliarty_scores_inperson$posmusicICC, granuliarty_scores_inperson$pospicICC)

##negative
cor.test(granuliarty_scores_inperson$negmusicICC, granuliarty_scores_inperson$negpicICC)

##combined
cor.test(granuliarty_scores_inperson$allmusicICC, granuliarty_scores_inperson$allpicICC)

#average differences
neg_averages_inperson <- neg_averages  %>% 
  filter(startsWith((Participant.Public.ID), 'TEAM'))

m1 <- lmer(data=neg_averages_inperson, mean ~ condition + (1|Participant.Public.ID))
summary(m1)

pos_averages_inperson <- pos_averages  %>% 
  filter(startsWith((Participant.Public.ID), 'TEAM'))

m1 <- lmer(data=pos_averages_inperson, mean ~ condition + (1|Participant.Public.ID))
summary(m1)

#arousal diff.
master_inperson <- master  %>% 
  filter(startsWith((Participant.Public.ID), 'TEAM'))
test <- lmer(data=master_inperson, scale(cor) ~ condition*arousal_dif + (condition*arousal_dif|Participant.Public.ID))
summary(test)

master_neg_inperson <- master %>%
  filter(startsWith((Participant.Public.ID), 'TEAM'))

test <- lmer(data=master_neg_inperson, scale(cor) ~ condition*arousal_dif + (condition*arousal_dif|Participant.Public.ID))
summary(test)
```
