...
BDI$BDI_average <- rowMeans(BDI[,-1], na.rm = TRUE)
...
```{r}
age <- df %>%
  select("SURVEY_CODE", "Q3")

names(age) <- c("SURVEY_CODE", "age")

age <- lapply(age, as.integer)

final_scores <- merge(final_scores, age, by.x = "SURVEY_CODE", by.y = "SURVEY_CODE", all.x = TRUE)

mean(final_scores$Q3.y, na.rm=TRUE)
range(final_scores$Q3.y, na.rm=TRUE)
```
