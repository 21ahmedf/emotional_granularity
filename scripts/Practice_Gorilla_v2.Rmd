...
```{r}
# List of emotions
neg_emotions <- c("Angry", "Disgusted", "Sad", "Scared", "Anxious")
pos_emotions <- c("Happy", "Satisfied", "Excited", "Joyful", "Proud")
...
filtered_music_neg <- lapply(seq_along(neg_emotions), function(i) {
  filter_and_rename(granularity_music_practice, neg_emotions[i], "Audio", keep_id = (i == 1))
})

filtered_music_pos <- lapply(seq_along(pos_emotions), function(i) {
  filter_and_rename(granularity_music_practice, pos_emotions[i], "Audio", keep_id = (i == 1))
})

filtered_pic_neg <- lapply(seq_along(neg_emotions), function(i) {
  filter_and_rename(granularity_pic_practice, neg_emotions[i], "Image", keep_id = (i == 1))
})

filtered_pic_pos <- lapply(seq_along(pos_emotions), function(i) {
  filter_and_rename(granularity_pic_practice, pos_emotions[i], "Image", keep_id = (i == 1))
})

# Combine the results
music_negative_practice <- bind_cols(filtered_music_neg) %>%
  na.omit()

music_positive_practice <- bind_cols(filtered_music_pos)

pic_negative_practice <- bind_cols(filtered_pic_neg)

pic_positive_practice <- bind_cols(filtered_pic_pos)

# Display the result
music_negative_practice
music_positive_practice
pic_negative_practice
pic_positive_practice
```
...
# Initialize results_practice DataFrame
results_practice <- expand.grid(ID = names(music_negative_list_practice))

# Compute ICC values for each subset and add to results_practice
results_practice$negmusicICC <- compute_icc(music_negative_list_practice, "negmusicICC")
results_practice$posmusicICC <- compute_icc(music_positive_list_practice, "posmusicICC")
results_practice$negpicICC <- compute_icc(pic_negative_list_practice, "negpicICC")
results_practice$pospicICC <- compute_icc(pic_positive_list_practice, "pospicICC")
...
for(i in seq_along(music_negative_list_practice)) {
  icc_result_practice <-  (music_negative_list_practice[[i]]) %>%
    select(-c("Participant.Public.ID")) %>%
 ICC()
  icc_value <- icc_result_practice$results %>%
    filter(type == "ICC3k") %>%
    pull(ICC)
  results_practice$negmusicICC[i] = 1 - icc_value
}
```
...
neg_graph <- ggplot(data = results_practice, aes(x = negpicICC, y = negmusicICC)) +
  geom_point() +
  stat_smooth(method = 'lm') +
...
mean_comparison <- results_practice %>%
