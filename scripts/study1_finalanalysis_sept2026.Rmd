...
```{r}
granularity_scores <- read.csv(file='../data/study1final_granularityscores.csv')
...
```{r}
#test for outliers 9/2/26

ggplot(granularity_scores, aes(y = negmusicICC)) +
  geom_boxplot()

ggplot(granularity_scores, aes(y = posmusicICC)) +
  geom_boxplot()

# Function to replace IQR outliers with NA
remove_outliers_iqr <- function(df,col_name) {
  ...
  df[[col_name]][x < lower_bound | x > upper_bound] <- NA
  df=df
}

granularity_scores <- remove_outliers_iqr(granularity_scores, "negmusicICC")
granularity_scores <- remove_outliers_iqr(granularity_scores, "negpicICC")
granularity_scores <- remove_outliers_iqr(granularity_scores, "posmusicICC")
granularity_scores <- remove_outliers_iqr(granularity_scores, "pospicICC")
...
#m1 <- lmer(data=master_ratingsperemotion, (value) ~ condition*rating_valence*stimulus_valence +  (rating_valence|Participant.Public.ID),control=lmerControl(optimizer="bobyqa")) #removed stim intercept to assist in model convergence and main random effect of stimulus_valence (explained least amt of variance), 8/13/26 -- and the main effect of condition and interaction between rating valence and condition

m1 <- lmer(data=master_ratingsperemotion, (value) ~ condition*rating_valence*stimulus_valence + (rating_valence|Participant.Public.ID),control=lmerControl(optimizer="bobyqa"))
car::Anova(m1,type="II" )
summary(m1)

#simple test of means (i.e., seeing pos rated pos, neg rated neg)
test(emmeans(m1, ~ condition*rating_valence*stimulus_valence, lmerTest.limit = 43840, lmer.df = "satterthwaite"))

emmeans::test(emmeans(m1, pairwise ~ condition*rating_valence*stimulus_valence,lmerTest.limit = 43840,lmer.df = "satterthwaite"))
...
ggplot(neg_averages,aes(x = Emotion, y = stim, fill = `Average\nRating`)) +
    geom_tile() +
...
pos_averages <- pos_averages %>%
  mutate(condition = recode(condition, music = "Positively-Valenced Music", pic = "Positiveley-Valenced Images"))

ggplot(pos_averages, aes(x = Emotion, y = stim, fill = `Average\nRating`)) +
    geom_tile() +
    #ggtitle(paste("Average Responses by Emotion and Stimulus for", group_name)) +
    xlab("Emotion") +
    ylab("Stimulus") +
...
ggsave('../heatmap_averagestudy2.png',averages_total_plot, dpi=1000,width=18, height=10.5)
```
...
ggsave('../figures/study2_posgrancor.png',dpi=1000, pos_granularity_plot, height=8)
...
ggsave('../figures/study2_neggrancor.png',dpi=1000, neg_granularity_plot, height=8)
...
ggsave('../figures/study2_magdifference.png',dpi=1000, mag_difference, width=14, height=12)
...
ggsave('../figures/study2_arousal_differentiation.png',dpi=1000, arousal_differentiation, width=14, height=12)
```