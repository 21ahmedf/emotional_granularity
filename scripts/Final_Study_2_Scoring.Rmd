...
#amedt corr across studies
````{r}
merged_study_2_recoded_with_aMEDT <- merged_study_2_recoded_with_aMEDT %>%
  select("SURVEY_CODE", "EDT.ability":"allpicICC")

amedt_master <- rbind(merged_recoded_with_aMEDT,merged_study_2_recoded_with_aMEDT)
```

##corr plot for poster
```{r}
# Select only the numeric columns for the correlation matrix
numeric_columns_recoded <- merged_df_study2_recoded[, sapply(merged_df_study2_recoded, is.numeric)] %>%
  select("Gold_musicaltraining", "BDI_total", "rdees_range", "rdees_differentiation", "rdees_total", "scaared_total", "negmusicICC":"pospicICC")
...
# Compute p-values
p_values <- cor.mtest(correlation_matrix_recoded)
#p_values_recoded <- cor.mtest(numeric_columns_recoded)
...
# Combine the folder path with the file name
output_file <- file.path(output_folder, "corrplotstudy2_postertest.pdf")

# Open a PNG device with the specified file path
pdf(output_file, width=10)
colnames(correlation_matrix_recoded) <- c('Gold-MSI: Musical Training', 'BDI Score', 'RDEES: Range Score', 'RDEES: Differentiation Score', 'Total RDEES Score', 'SCAARED Score',  "Negative Emotional Granularity Score: Music", "Positive Emotional Granularity Score: Music", 
                           "Negative Emotional Granularity Score: Pictures","Positive Emotional Granularity Score: Pictures")
rownames(correlation_matrix_recoded) <- c('Gold-MSI: Musical Training', 'BDI Score', 'RDEES: Range Score', 'RDEES: Differentiation Score', 'Total RDEES Score', 'SCAARED Score',  "Negative Emotional Granularity Score: Music", "Positive Emotional Granularity Score: Music", 
                           "Negative Emotional Granularity Score: Pictures","Positive Emotional Granularity Score: Pictures")

# Generate the corrplot
corrplot(correlation_matrix_recoded, method = "circle", type = "upper", 
         p.mat = p_values, sig.level = 0.05, insig = "blank", show.diag = FALSE,
         tl.col = "black", tl.srt = 45, addCoef.col = NULL,tl.pos = 'n',
         col = colorRampPalette(c("blue", "white", "red"))(200))
