...
panas <- read.csv(file='../data/redcap/TEAMYA_PANASX.csv')
```
...
BDI_study_2$BDI_average <- rowMeans(BDI_study_2[,-1], na.rm = TRUE)

final_scores_study2 <- merge(final_scores_study2, BDI_study_2[, c(1, 22,23)], by.x = "SURVEY_CODE", by.y = "SURVEY_CODE", all.x = TRUE)

hist(final_scores_study2$BDI_average)
```
...
final_scores_study2$scaared_average <- rowMeans(scaared, na.rm = TRUE)
```

#PANAS
```{r}
PANAS <- panas %>%
  dplyr::select("Participant.ID","Cheerful":"Dissatisfied.with.self")

PANAS[PANAS == '1: very slightly or not at all'] <- 1

PANAS[PANAS == '5: extremely'] <- 5


PANAS <- pivot_longer(PANAS,"Cheerful":"Dissatisfied.with.self", values_to = "Response", names_to = "emotion")

PANAS$Response <- as.numeric(PANAS$Response) 

PANAS <- PANAS %>%
  drop_na()

PANAS_ID <- unique(PANAS$Participant.ID) %>%
  as.data.frame()

#getting ICC
# List of "general positive emotions" 
emotions <- list(
  neg = c("Distressed", "Upset", "Ashamed", "Guilty", "Hostile",
          "Irritable", "Jittery", "Nervous", "Scared", "Afraid"),
  pos = c("Active", "Alert", "Determined", "Enthusiastic", "Excited",
          "Inspired", "Interested", "Proud", "Strong","Attentive")
)

# Function to filter and rename
filter_and_rename <- function(data, emotion, keep_id = FALSE) {
  columns_to_select <- if (keep_id) {
    c("Participant.ID", "Response")
  } else {
    c("Response")
  }
...
apply_filter <- function(data, emotions) {
  lapply(seq_along(data$Participant.ID), function(i) {
    filter_and_rename(data, emotions[i], keep_id = (i == 1))
  }) %>% 
  bind_cols() %>% 
   na.omit()
}
...
negative <- PANAS %>%
  filter(emotion %in% emotions$neg) %>%
  select(-emotion)

positive <- PANAS %>%
  filter(emotion %in% emotions$pos)
...
negative_list <- split(negative, PANAS_ID$.)
positive_list <- split(positive, PANAS$Participant.ID)

###
PANAS_ID$negICC <- compute_icc_fisher_recode(negative_list)
...
final_scores_study2$age <- age$age

sex <- surveys_study2 %>%
  dplyr::select("sex")

final_scores_study2$sex <- sex$sex

gender_sex <- surveys_study2 %>%
  dplyr::select("gender___1":"gender___10") #figure out how to score this + race / ethnicity data, add to scoring here and then merge with data in "8_Survey analyses" for this + study 1 to get the right participants (need to do the same of 6_qualtrics survey)

merge(study2, gender_sex, by.x="ID", by.y="participant_id", all.x = TRUE)
